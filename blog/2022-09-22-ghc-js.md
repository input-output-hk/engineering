...
merged into the master branch on XXXXXX November 2022. This post should answer
most of the questions you may have about this backend.
...
4. GHCJS's support for plugins hasn't been ported. The implementation was
   ...
   (see #14335): GHC assumes there is a single global unit environment.
...
refactored the code to avoid them.

One notorious example is that we switched from Text to GHC's ShortText (which
provides a similar API) and finally to GHC's FastString in most cases (which is
more performant in many cases). We also replaced a lot of code using lenses with
its understandable counterpart (which is more common in GHC). We also switched
to use GHC's pretty-printer.
...
We've spent quite some time trying to run the testsuite on CI.
Sadly Hadrian doesn't quite support this yet (more concretely, it doesn't
...
There are two ways to fix libraries that use C sources.
